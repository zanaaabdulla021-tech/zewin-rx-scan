...
class _RootShellState extends State<_RootShell> {
  ...
  Widget build(BuildContext context) {
    ...
    final navItems = <BottomNavigationBarItem>[
      const BottomNavigationBarItem(icon: Icon(Icons.document_scanner_outlined), label: 'سکان'),
      const BottomNavigationBarItem(icon: Icon(Icons.history), label: 'مێژوو'),
      if (isAdmin) const BottomNavigationBarItem(icon: Icon(Icons.bar_chart_outlined), label: 'ڕاپۆرت'),
      if (isAdmin) const BottomNavigationBarItem(icon: Icon(Icons.admin_panel_settings_outlined), label: 'بەڕێوەبردن'),
      const BottomNavigationBarItem(icon: Icon(Icons.person_outline), label: 'هەژمار'),
    ];
    ...
    return Scaffold(
      appBar: AppBar(
        title: Column(
          ...
          children: [
            const Text('خوێندنەوەی ڕەسیت'),
            if (user != null)
              Text(
                '${user.branchName ?? "بێ لق"} · ${user.email}',
